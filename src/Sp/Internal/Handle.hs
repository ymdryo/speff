...
inject :: (KnownSubset es' es, Monad m) => Eff es' a -> Eff es a
...
rearrange :: (Subset es es', Monad m) => Eff es a -> Eff es' a
rearrange = alter Rec.extract
{-# INLINE rearrange #-}

-- | Like 'rearrange' but with the prefixes explicitly supplied via @TypeApplications@.
rearrangeN :: ∀ es' es'' es m a. (KnownList es'', KnownSubset es' (es'' ++ es), Monad m) => Eff (es' ++ es) a -> Eff (es'' ++ es) a
